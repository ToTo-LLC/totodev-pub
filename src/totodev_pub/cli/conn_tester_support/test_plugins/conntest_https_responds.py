...
"""
HTTPS TCP connectivity test plugin.

This module provides TCP connectivity testing for HTTPS port 443,
used as a prerequisite for HTTPS connection tests.
"""
...
from conn_tester_support.core import TestTypeBase, DEFAULT_TCP_TIMEOUT, HTTPS_PORT
from conn_tester_support.models import TestMetadata, TestResult, ConfigurationError


class TestTypeHttpsResponds(TestTypeBase):
    """TCP connection test for HTTPS port 443"""
    
    @classmethod
    def describe_self(cls) -> TestMetadata:
        return TestMetadata(
            description="Tests TCP connectivity to HTTPS port 443",
            config_fields={
    ...
    def run_test(self, logger: Optional[Any] = None) -> TestResult:
        ...
        port = self.config.get("port", HTTPS_PORT)
...
TestTypeBase.register_test(
    test_class=TestTypeHttpsResponds,
    shortname="https-responds",
    is_public=False,
    description="Tests TCP connectivity to HTTPS port 443"
)
