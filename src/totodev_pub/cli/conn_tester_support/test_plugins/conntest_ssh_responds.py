...
"""
SSH TCP connectivity test plugin.

This module provides TCP connectivity testing for SSH port 22,
used as a prerequisite for SSH connection tests.
"""
...
from conn_tester_support.core import TestTypeBase, DEFAULT_TCP_TIMEOUT, SSH_PORT
from conn_tester_support.models import TestMetadata, TestResult, ConfigurationError


class TestTypeSshResponds(TestTypeBase):
    """TCP connection test for SSH port 22"""
    
    @classmethod
    def describe_self(cls) -> TestMetadata:
        return TestMetadata(
            description="Tests TCP connectivity to SSH port 22",
            config_fields={
                "host": "Target hostname or IP address",
                "port": "Target port number (default: 22)",
        ...
        try:
            ...
            if result == 0:
                ...
                return TestResult(
                    ...
                    advice=["Check if SSH service is running", "Verify firewall settings"],
                    extra_detail={
                        "tcp_connect": {
                            "ok": False,
                            "remote_ip": host,
...
TestTypeBase.register_test(
    test_class=TestTypeSshResponds,
    shortname="ssh-responds",
    is_public=False,
    description="Tests TCP connectivity to SSH port 22"
)
