...
"""Tests for the case briefing generator (case_briefing mini-spec).
...
# networkx is an optional dependency (not required for core FSM usage), and every
# briefing path goes through FsmChainSpec.to_networkx() via collect(), so the whole
# module skips at collection time on a core-only install (same pattern as the
# to_networkx() tests in test_state_chain_parser.py).
pytest.importorskip("networkx")

from totodev_pub.folder_backed_case import FolderBackedCase
from totodev_pub.folder_backed_case_support import case_briefing
...
from totodev_pub.folder_backed_case_support.case_briefing import (
    CaseBriefingOptions, collect, generate_case_briefing, render_markdown, to_mermaid,
)
...
def test_docstring_mode_none_drops_all_docs():
    doc = collect(SampleCase, options=CaseBriefingOptions(docstring_mode="none"))
    assert doc.class_doc is None
...
def test_to_mermaid_flowchart_style_with_wildcard_hub():
    doc = collect(SampleCase, options=CaseBriefingOptions(wildcard_pseudo_state=True))
    mermaid = to_mermaid(doc.fsm_graph, style="flowchart")
    assert mermaid.startswith("flowchart TD")
    assert case_briefing._MERMAID_WILDCARD_ID in mermaid
...
def test_to_mermaid_omits_wildcard_fanout_by_default():
    ...
    mermaid = to_mermaid(doc.fsm_graph, style="flowchart")
    assert case_briefing._MERMAID_WILDCARD_ID in mermaid
    assert f'{case_briefing._MERMAID_WILDCARD_ID} ==>|"cancel"| cancelled' in mermaid
...
def test_render_markdown_default_diagram_hides_wildcard_fanout():
    ...
    assert f'{case_briefing._MERMAID_WILDCARD_ID} ==>|"cancel"| cancelled' in text
    assert 'new ==>|"cancel"| cancelled' not in text
    # Triggers table matches: abstract ``*`` only, no fan-out rows / Wildcard column.
    assert "| * | cancelled |" in text
    assert "| new | cancelled |" not in text
    assert "Wildcard" not in text

# ---------------------------------------------------------------------------
# render_markdown() / generate_case_briefing()
# ---------------------------------------------------------------------------
...
def test_render_markdown_starts_with_generation_stamp(monkeypatch):
    ...
    monkeypatch.setattr(case_briefing, "datetime", _FakeDateTime, raising=False)

    text = generate_case_briefing(SampleCase)
    first_line = text.splitlines()[0]
    assert first_line.startswith("<!-- Case briefing generated by ")
    assert first_line.endswith(" -->")
    assert "totodev_pub.folder_backed_case_support.case_briefing" in first_line
...
def test_render_markdown_surfaces_bound_method_names():
    ...
    # Assertions: flat Method | Description table (no per-state headers)
    assert "| Method | Description |" in text
    assert "| `case_assert_done_totals_balance()` |" in text
    assert "| `case_assert_reviewing_has_ticket()` |" in text
    assert "The closed ticket's recorded total matches the invoice." in text
    assert "- **`case_assert_" not in text  # old per-state list form gone
    assert "### done" not in text  # done had assertions only; no state subsection
...
def test_render_asset_aliases_flags_column_and_many_path_italic():
    text = generate_case_briefing(SampleCase)
    assert "| Alias | Path | Loader | Trust states | Flags |" in text
    # keep=True → sparse "keep"; many=False → plain (non-italic) path code span
    assert "| ticket | `ticket.yaml` | Path | `done`, `reviewing` | keep |" in text
    assert "| Keep |" not in text
    assert "| Many |" not in text
    # many=True italicizes the whole path cell (glob cue); keep stays in Flags only
    assert case_briefing._fmt_asset_path("attachments/*", many=True) == "*`attachments/*`*"
    assert case_briefing._fmt_asset_path("x.yaml", many=False) == "`x.yaml`"
    assert case_briefing._fmt_asset_flags(keep=False) == ""
...
def test_render_states_table_sparse_bools():
    text = generate_case_briefing(SampleCase)
    # Header collapses initial/default/terminal; default initial is bold.
    assert (
...
def test_render_states_can_go_to_respects_wildcard_fanout_flag():
    """Fan-out on/off should not change per-state dests when the only wildcard
    effect is a single shared destination — both paths yield ``cancelled``."""
    default = generate_case_briefing(SampleCase)
...
def test_render_trigger_guards_merged_declaration_order():
    ...
    assert "| Source | Dest | Auto | Guards | Chokes | Soft timeout |" in text
    assert "Wildcard" not in text
    # recheck (auto + method): method then implied @FAIL (italic, not code)
    assert "`guard_funded()`, *@FAIL<1*" in text
    assert "`@FAIL<1`" not in text  # implied must not look author-declared
    assert "Chokes: " not in text  # moved into table
    assert "Soft timeout:" not in text  # column, not a prose line
    assert "finance-api" in text
    assert "5s (default)" in text
    # Wildcard triggers: abstract ``*`` row only (fan-out omitted by default)
    assert "| * | cancelled |" in text
    assert "| new | cancelled |" not in text


def test_render_trigger_table_wildcard_fanout_swaps_star_for_concrete():
    text = generate_case_briefing(
        SampleCase, options=CaseBriefingOptions(include_wildcard_expanded_edges=True),
    )
    assert "| new | cancelled |" in text
    assert "| reviewing | cancelled |" in text
...
def test_render_markdown_sections_are_individually_toggleable():
    ...
    text = generate_case_briefing(SampleCase, options=options)
...
def test_render_markdown_omits_hooks_section_when_none_overridden():
    text = generate_case_briefing(UndocumentedCase)
    assert "## Overridden Lifecycle Hooks" not in text
...
def test_cli_main_smoke(monkeypatch, capsys):
    ...
    monkeypatch.setattr(case_briefing.importlib, "import_module", lambda name: fake_module)

    rc = case_briefing.main(["fake_module:SampleCase", "--no-diagram"])
...
def test_cli_main_rejects_bad_target_syntax(capsys):
    with pytest.raises(SystemExit) as exc_info:
        case_briefing.main(["no-colon-here"])
...
def test_cli_main_rejects_non_case_class(monkeypatch, capsys):
    ...
    with pytest.raises(SystemExit) as exc_info:
        case_briefing.main(["fake_module:NotACase"])
    assert exc_info.value.code == 2
...
def test_to_mermaid_state_marks_manual_edges_with_label_marker():
    ...
    Prefer flowchart for human-facing docs (see CaseBriefingOptions.diagram_style)."""
    doc = collect(SampleCase)
...
def test_fmt_dwell_duration_summary_units(secs, expected):
    assert case_briefing._fmt_dwell_duration(secs) == expected


def test_fmt_fact_guard_dwell_uses_summary_units():
    assert case_briefing._fmt_fact_guard(
...
def test_fmt_duration_soft_timeout_stays_exact():
    ...
    assert case_briefing._fmt_duration(120) == "2m"
    assert case_briefing._fmt_duration(90) == "90s"  # 90 does not divide cleanly into minutes
    assert case_briefing._fmt_duration(14 * 86400 - 20) == f"{14 * 86400 - 20:g}s"
